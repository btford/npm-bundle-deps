  ...
  function getTarForPackageJson (cb) {
    ...
    checksum.file(fileToCheck, function (err, sum) {
      ...
      if (fs.existsSync(outputFile)) {
        ...
        if (!generatedTars[outputFile]) {
          log(outputFile + ' will get generated');
          var defer = Q.defer();
          generatedTars[outputFile] = defer.promise;
          ...
          npmInstall(npmDir, function (err, data) {
            ...
            tarNodeModules(outputFile, function () {
              defer.resolve();
              delete generatedTars[outputFile];
              cb(outputFile)
            });
          });
        } else {
          log(outputFile + ' is already being generated by someone else');
          generatedTars[outputFile].then(function () {
            cb(outputFile)
          });
        }
      }
